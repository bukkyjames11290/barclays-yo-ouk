...
export const useTransferLogic = (userAccounts: (Account & { id: string })[]) => {
  ...
  const handlers: TransferHandlers = {
    ...
    setAccountNumber: val => {
      ...
      const cleaned = val.replace(/[^a-zA-Z0-9 ]/g, '').slice(0, 30);
  ...
  const hasInsufficientFunds = transferAmount > 0 && selectedFromAccount ? transferAmount > selectedFromAccount.balance : false;

  const isFormValid: boolean =
    !!amount && transferAmount > 0 && !!selectedBank && accountNumber.trim().length >= 5 && accountNumber.trim().length <= 30 && routingNumber.length >= 5 && !hasInsufficientFunds;
