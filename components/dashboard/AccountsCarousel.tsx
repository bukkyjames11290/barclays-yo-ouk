      ...
      <div className="px-5 flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1.5">
          {accounts.map((_, index) => (
            <div key={index} className={`w-2 h-2 rounded-full transition-colors ${index === 0 ? 'bg-[#0076b6]' : 'bg-gray-700'}`} />
          ))}
        </div>
      </div>
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto px-5 pb-2 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
