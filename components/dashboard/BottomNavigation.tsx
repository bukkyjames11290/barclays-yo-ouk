...
import { Home, CreditCard, ArrowLeftRight, User, History } from 'lucide-react';
        ...
        {navItems.map(item => {
          ...
          return (
            <Link
              key={item.label}
              href={item.path}
              className={`flex flex-col items-center gap-1 px-4 py-2 rounded-xl transition-all ${isActive ? 'text-[#0076b6]' : 'text-gray-500 hover:text-gray-700'}`}
            >
              <div className={`p-1.5 rounded-lg transition-colors ${isActive ? 'bg-[#0076b6]/10' : ''}`}>
