...
const TransferForm = ({ state, handlers, validation, selectedFromAccount, displayAccounts = [], onContinue }: TransferFormProps) => {
  return (
    <>
...
const AmountInput = ({ amount, hasError, availableBalance, onChange }: { amount: string; hasError: boolean; availableBalance?: number; onChange: (val: string) => void }) => (
  <section className="px-5 mb-6 animate-fade-up stagger-2">
    <div className={`banking-card text-center py-8 ${hasError ? 'border-2 border-destructive' : ''}`}>
      ...
      <div className="flex items-center justify-center gap-1">
        <span className={`text-4xl font-bold ${hasError ? 'text-[#ef4343]' : 'text-foreground'}`}>£</span>
        <input
          type="text"
          value={amount}
          onChange={e => onChange(e.target.value)}
          placeholder="0.00"
          className={`text-4xl font-bold bg-transparent outline-none w-40 text-center placeholder:text-gray-500/50 ${hasError ? 'text-[#ef4343]' : 'text-foreground'}`}
        />
      </div>
      {hasError && (
        <div className="flex items-center justify-center gap-2 mt-3 text-[#ef4343]">
          <AlertCircle className="w-4 h-4" />
          <p className="text-sm font-medium">Insufficient balance. Available: £{availableBalance?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
      )}
    </div>
  </section>
...
const FromAccountSection = ({ accounts, selectedAccountId, onSelect }: { accounts: (Account & { id: string })[]; selectedAccountId: string; onSelect: (id: string) => void }) => (
      ...
      {accounts.map(account => (
        <button
          ...
          className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all ${
            selectedAccountId === account.id ? 'bg-[#0076b6]/10 border-2 border-[#0076b6]' : 'bg-white shadow-card border-2 border-transparent'
          }`}
        >
          <div className="p-2.5 rounded-md bg-[#0076b6] text-white">
...
const RecipientDetailsSection = ({
  ...
  <section className="px-5 mb-6 animate-fade-up stagger-4">
    ...
    <div className="space-y-4 mt-3">
      ...
      <FormInput label="Account Number" value={accountNumber} onChange={onAccountNumberChange} placeholder="Enter account number" />
      ...
      <FormInput label="Memo (Optional)" value={memo} onChange={onMemoChange} placeholder="What's this transfer for?" />
    </div>
  </section>
);
