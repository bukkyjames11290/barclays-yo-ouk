...
const TransactionCodeDialog = ({ open, onConfirm, onCancel, isProcessing }: TransactionCodeDialogProps) => {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-sm mx-auto bg-white">
        <DialogHeader className="text-center items-center">
          <div className="p-4 rounded-full bg-[#0076b6]/10 mb-4">
            <Lock className="w-8 h-8 text-[#0076b6]" />
          </div>
          <DialogTitle className="text-xl font-bold text-foreground">Verify Transfer</DialogTitle>
          <DialogDescription className="text-gray-500 mt-2">Enter your 4-digit transaction code to confirm this transfer</DialogDescription>
        </DialogHeader>

        <div className="mt-6 space-y-4">
          <div>
            <label className="text-xs font-medium text-gray-500 mb-2 block">Transaction Code</label>
            <input
              type="text"
              value={code}
              ...
              className={`w-full px-4 py-4 text-2xl font-bold text-center rounded-md bg-white border-2 focus:ring-1 outline-none transition-all text-foreground placeholder:text-gray-400 disabled:opacity-50 ${
                error ? 'border-[#ef4343] focus:border-[#ef4343] focus:ring-[#ef4343]' : 'border-gray-200 focus:border-[#0076b6] focus:ring-[#0076b6]'
              }`}
            />
            {error && (
