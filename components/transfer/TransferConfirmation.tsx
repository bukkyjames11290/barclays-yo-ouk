...
const TransferConfirmation = ({ amount, selectedFromAccount, selectedBankInfo, accountNumber, routingNumber, memo, isProcessing, onConfirm, onCancel }: TransferConfirmationProps) => {
    ...
    <div className="min-h-screen bg-background pb-28">
      <header className="px-5 pt-5 pb-6 animate-fade-up">
        <div className="flex items-center gap-4">
          <button onClick={onCancel} className="p-2 -ml-2 rounded-full hover:bg-[#0076b6] transition-colors">
      ...
      <ConfirmationDetails selectedFromAccount={selectedFromAccount} selectedBankInfo={selectedBankInfo} accountNumber={maskedAccountNumber} routingNumber={routingNumber} memo={memo} />

      {/* <EstimatedArrival /> */}

      <ActionButtons isProcessing={isProcessing} onConfirm={onConfirm} onCancel={onCancel} />
...
//     <div className="bg-[#0076b6]/10 rounded-2xl p-4 flex items-start gap-3">
//       <div className="p-1 rounded-full bg-[#0076b6]">
...
const ActionButtons = ({ isProcessing, onConfirm, onCancel }: { isProcessing: boolean; onConfirm: () => void; onCancel: () => void }) => (
  <section className="px-5 space-y-3 animate-fade-up stagger-4">
    ...
    <button
      onClick={onCancel}
      disabled={isProcessing}
      className="w-full py-4 cursor-pointer rounded-2xl bg-[#0076b6]/70 text-white font-semibold text-lg transition-colors hover:bg-[#0076b6] active:scale-[0.98] disabled:opacity-50"
    >
      Cancel
    </button>
  </section>
