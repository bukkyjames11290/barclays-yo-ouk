...
const accounts = [
  ...
  { id: 2, name: 'Savings Account', number: '****7890', balance: 15234.5 }
];
    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <section className="px-5 mb-6 animate-fade-up stagger-1">
        <div className="flex items-center gap-2 mb-4">
          {[1, 2, 3].map(s => (
            <div key={s} className={`flex-1 h-1 rounded-full transition-colors ${s <= step ? 'bg-[#0076b6]' : 'bg-border'}`} />
          ))}
        </div>
      </section>
      ...
      {step === 1 && (
        <>
            ...
            <div className="space-y-3">
              {accounts.map(account => (
                <button
                  ...
                  className={`w-full banking-card flex items-center justify-between text-left transition-all ${selectedAccount === account.id ? 'ring-2 ring-primary' : ''}`}
                >
                  <div>
                    <p className="font-medium text-foreground">{account.name}</p>
                    <p className="text-sm text-gray-500">{account.number}</p>
                  </div>
                  <p className="font-semibold text-foreground">${account.balance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="px-5 animate-fade-up stagger-4">
            <button onClick={() => setStep(2)} disabled={!selectedAccount || !amount} className="w-full py-4 rounded-2xl bg-[#223e99] text-[#0076b6]-foreground font-semibold disabled:opacity-50">
              Continue
            </button>
          </section>
        </>
      ...
      {step === 2 && (
        <>
          <section className="px-5 mb-6 animate-fade-up">
            <div className="banking-card mb-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <p className="text-sm text-gray-500">Position check on a flat, dark surface with good lighting</p>
              </div>
            </div>
          ...
          <section className="px-5 animate-fade-up stagger-2">
            <button onClick={() => setStep(3)} disabled={!frontCaptured || !backCaptured} className="w-full py-4 rounded-2xl bg-[#223e99] text-[#0076b6]-foreground font-semibold disabled:opacity-50">
              Submit Deposit
            </button>
          </section>
        </>
      ...
      {step === 3 && (
        <section className="px-5 animate-fade-up">
          <div className="banking-card text-center py-12">
            ...
            <p className="text-gray-500mb-6">Your deposit of ${parseFloat(amount).toFixed(2)} is being processed</p>
            <div className="bg-[#0076b6] rounded-xl p-4 mb-6">
            ...
            <Link href="/" className="inline-block w-full py-4 rounded-2xl bg-white border border-gray-200 font-semibold text-foreground hover:bg-[#0076b6] transition-colors">
              Back to Home
            </Link>
          </div>
        </section>
