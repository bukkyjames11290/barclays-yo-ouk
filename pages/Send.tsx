...
const Send = () => {
  ...
  const filteredContacts = recentContacts.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()) || c.email.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="min-h-screen bg-background pb-28">
            ...
            {favorites.map(contact => (
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-2xl min-w-[80px] transition-all ${
                  selectedContact === contact.id ? 'bg-[#0076b6] text-[#0076b6]-foreground' : 'bg-white hover:bg-[#0076b6]'
                }`}
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold ${
                    selectedContact === contact.id ? 'bg-[#0076b6]-foreground/20' : 'bg-[#223e99] text-[#0076b6]-foreground'
                  }`}
                >
                  {contact.avatar}
                </div>
                <span className="text-xs font-medium truncate w-full text-center">{contact.name.split(' ')[0]}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <div className="banking-card !p-0 overflow-hidden divide-y divide-border">
          {others.map(contact => (
            <button
              key={contact.id}
              onClick={() => setSelectedContact(contact.id)}
              className={`w-full flex items-center gap-4 p-4 transition-colors text-left ${selectedContact === contact.id ? 'bg-[#0076b6]/10' : 'hover:bg-[#0076b6]/50'}`}
            >
              <div className="w-12 h-12 rounded-full bg-[#223e99] flex items-center justify-center text-[#0076b6]-foreground font-bold">{contact.avatar}</div>
              <div className="flex-1">
                <p className="font-medium text-foreground">{contact.name}</p>
                <p className="text-sm text-gray-500">{contact.email}</p>
              </div>
              {selectedContact === contact.id && (
                <div className="w-6 h-6 rounded-full bg-[#0076b6] flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-[#0076b6]-foreground" />
                </div>
              )}
            </button>
          ))}
      ...
      <section className="px-5 py-6 animate-fade-up stagger-5">
        <button
          ...
          className="w-full py-4 rounded-2xl bg-[#223e99] text-[#0076b6]-foreground font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
        >
          Send Money
        </button>
      </section>
